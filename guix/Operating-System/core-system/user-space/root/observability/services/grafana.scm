...
(define-module (core-system user-space root observability services grafana)
  ...
  #:use-module (core-system user-space root observability build utils)
    ...
    (let* ((auto-start?
           ...
           (container-config
            (oci-container-configuration
             ...
             (log-file log-file)
             (extra-arguments (list (if (eq? runtime 'podman)
                                    (string-append
                                     "--userns=keep-id:uid="
                                     grafana-user-uid)
                                    (string-append
...
(define oci-grafana-service-type
  (service-type (name 'grafana)
                ...
                (description
                 "This service runs Grafana in a Docker container.")))
