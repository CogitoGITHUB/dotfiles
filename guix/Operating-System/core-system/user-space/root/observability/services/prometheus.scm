...
(define-module (core-system user-space root observability services prometheus)
  ...
  #:use-module (core-system user-space root observability services configuration)
               ...
               `("--web.enable-lifecycle"
                 ...
                 ,(string-append "--storage.tsdb.retention.time="
                  ...
                  ,@(if (and retention-size (maybe-value-set? retention-size))
                        (list (string-append "--storage.tsdb.retention.size="
                                       retention-size))
                        '())))
...
(define oci-prometheus-service-type
  (service-type (name 'prometheus)
                ...
                (description
                 "This service runs Prometheus in a Docker container.")))
...
(define %default-blackbox-exporter-file
...
(define-configuration oci-blackbox-exporter-configuration
  ...
  (file
   (file-like %default-blackbox-exporter-file)
  ...
  (service-type (name 'blackbox-exporter)
                (extensions (list (service-extension oci-service-type
                                                     (lambda (config)
                                                       (oci-extension
                                                        (containers
                                                         (oci-blackbox-exporter-configuration->oci-container-configuration config)))))
                                  (service-extension activation-service-type
                                                     %blackbox-exporter-activation)))
                (default-value (oci-blackbox-exporter-configuration))
                (description
                 "This service runs Blackbox Exporter in a Docker container.")))
