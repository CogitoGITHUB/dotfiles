...
(define-module (core-system user-space root grafana-stack sops derive)
...
(define* (generate-host-key age-key-file
      ...
      (with-imported-modules (source-module-closure
                              '((core-system user-space root grafana-stack sops build age)
                                (core-system user-space root grafana-stack sops build gnupg))
                              #:select? sops-module-name?)
        (program-file
         "generate-host-key"
         #~(begin
             (use-modules (core-system user-space root grafana-stack sops build age)
                          (core-system user-space root grafana-stack sops build gnupg)
                          (ice-9 format)
                          (srfi srfi-11))
