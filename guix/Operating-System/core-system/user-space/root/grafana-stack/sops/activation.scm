...
(define-module (core-system user-space root grafana-stack sops activation)
  #:use-module (gnu packages gnupg)
  #:use-module (guix gexp)
  #:use-module (guix modules)
  #:use-module (guix records)
  #:use-module (core-system user-space root grafana-stack sops derive)
...
(define (wait-for-secrets runtime-state)
  ...
  (match-record runtime-state <sops-runtime-state>
    ...
    (let ((sops-secrets-names
      ...
      (with-imported-modules (source-module-closure
                              '((core-system user-space root grafana-stack sops build activation)
                                (core-system user-space root grafana-stack sops build utils))
                              #:select? sops-module-name?)
        #~(begin
            (use-modules (core-system user-space root grafana-stack sops build activation)
                         (core-system user-space root grafana-stack sops build utils)
                         (ice-9 format))
