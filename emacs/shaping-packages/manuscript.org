...
:PACKAGE:  denote
:STRAIGHT: t
:DESCRIPTION: Advanced note-taking system with .live files, silos, and multi-format export
:END:
...
;; ════════════════════════════════════════════════════════════
;; This system uses a dual-directory approach:
;; - silos/    : Source .live files (org-mode with custom extension)
;; - formats/  : Exported files organized by format type
;;
;; Structure:
;; ~/testing-manuscripts/
;;   ├── silos/
...
** Multi-Format Export System :config:
#+begin_src emacs-lisp
;; ════════════════════════════════════════════════════════════
;; § EXPORT ENGINE — MULTI-FORMAT PUBLISHING
;; ════════════════════════════════════════════════════════════
;; Exports .live files to multiple formats while maintaining
;; the same directory structure in formats/ as in silos/
;;
;; Example:
;; silos/work/project-a/note.live
;;   → formats/pdf/work/project-a/note.pdf
;;   → formats/html/work/project-a/note.html
;;   → formats/tex/work/project-a/note.tex
;;   etc.
...
Exports to: txt, tex, html, md, pdf, docx, beamer."
  (let* ((relative-silo (shapeshift/get-relative-silo-path live-file))
    ...
    (with-current-buffer (find-file-noselect live-file)
      ...
      (dolist (format-pair formats)
        (let* ((ext (car format-pair))
          ...
          (condition-case err
              (progn
                (funcall export-fn)
                ;; Move exported file from silos to formats directory
      ...
      ;; Also keep an .org copy in formats/org
...
#+begin_src emacs-lisp
;; ════════════════════════════════════════════════════════════
;; § CLEANUP — MAINTAIN PURITY OF SILOS
;; ════════════════════════════════════════════════════════════
;; The silos directory should only contain .live source files.
;; This function removes any exported files (pdf, tex, html, etc)
;; that may have been created in the silos directory.
...
** Auto-Export Hook :config:
#+begin_src emacs-lisp
;; ════════════════════════════════════════════════════════════
;; § AUTO-EXPORT — CONTINUOUS PUBLISHING
;; ════════════════════════════════════════════════════════════
;; Automatically export .live files to all formats on save,
;; then clean up any stray exports in the silos directory.
...
** Org-Capture Integration :config:
#+begin_src emacs-lisp
;; ════════════════════════════════════════════════════════════
;; § CAPTURE TEMPLATES — RAPID NOTE CREATION
;; ════════════════════════════════════════════════════════════
;; Use org-capture to create new .live files with all metadata
;; and automatic linking capabilities.
...
;; ════════════════════════════════════════════════════════════
;; Real-time PDF preview in split window with auto-refresh.
;; Uses idle timer to avoid excessive re-rendering.
           ...
           (when (buffer-file-name)
             (let* ((org-file (buffer-file-name))
               ...
               (when (file-exists-p pdf-in-formats)
                 ...
                 (message "PDF updated ✓"))
               ;; Clean up any PDF in silos directory
...
(with-eval-after-load 'evil
  (evil-define-key 'normal org-mode-map
    ...
    ;; ──────────────────────────────────────────────────────
    (kbd "SPC n n")   (lambda () (interactive) (org-capture nil "n"))
    (kbd "SPC n d")   (lambda () (interactive) 
                        (dired shapeshift/denote-silos-dir))
    (kbd "SPC n e")   (lambda () (interactive) 
    ...
    (kbd "SPC m e")   'shapeshift/export-to-all-formats
    (kbd "SPC m c")   'shapeshift/cleanup-silos))
#+end_src
