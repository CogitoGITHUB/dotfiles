...
CONTEXTS/KIND/LABEL come from #+CONTEXTS:/#+KIND:/#+LABEL: file
keywords above the first heading, not an in-drawer property anymore.
...
for backward compatibility. A legacy :PROPERTIES:...:END: drawer
right after the heading is tolerated and skipped (its CONTEXTS/KIND/
LABEL lines, if any, are ignored -- migrate the file to keywords)."
  (with-temp-buffer
    (insert-file-contents file)
    (goto-char (point-min))
        ...
        (while (and (not (eobp))
          ...
          (when (looking-at "^#\\+\\([A-Za-z_-]+\\):[ \t]*\\(.*\\)$")
            (let ((kw (downcase (match-string 1))))
              (when (member kw '("contexts" "kind" "label"))
                (setq file-plist
                      (plist-put file-plist (intern (concat ":" kw))
                                 (string-trim (match-string 2)))))))
          (forward-line 1))
        (goto-char (point-min))
              ...
              (cond
               ((looking-at "^\\* \\([A-Z][A-Z0-9_]*\\)[ \t]*$")
                ...
                (setq specs (my/manifolding-atlas-org-prompt--parse-flush
                      ...
                      plist (copy-sequence file-plist)
