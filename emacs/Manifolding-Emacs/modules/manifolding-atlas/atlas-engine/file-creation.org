  ...
  (let* ((fname (my/manifolding-atlas-prompt-file-name title nil))
    ...
    (let ((org-capture-templates
      ...
      (let* ((capture-buf (current-buffer))
             ...
             (template (my/manifolding-atlas-prompt-template--choose-with-live-capture-preview capture-buf preview-start preview-end)))
        ))
    note))
...
Plain `completing-read' with live preview in *Template Preview* side window."
...
** my/manifolding-atlas-prompt-template--choose-with-live-capture-preview
Why: Live preview updating the capture buffer itself, no side window.

#+begin_src emacs-lisp
(defun my/manifolding-atlas-prompt-template--choose-with-live-capture-preview (capture-buf preview-start preview-end)
  "Ask for TEMPLATE, live-previewing inside CAPTURE-BUF as cursor moves.
...
(defun my/manifolding-atlas-org-prompt--fragment-live (key plist val)
  "Live-kind properties write the ref-note's id-link as a plain
property VALUE inside the note's existing :PROPERTIES: drawer --
never as a separate transcluded block outside it."
  (let ((family (intern
                 (downcase
