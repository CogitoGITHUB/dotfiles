...
** Boot sync
#+begin_src emacs-lisp
(defun my/manifolding-atlas--sync-drawer-order-new-keys ()
  "Append prompt keys missing from drawer-order.org, then apply.
Known spec :key strings minus current order headings, sorted for
determinism, appended as `** KEY' lines without touching existing
order. Saves the file once via `with-temp-file' (no hooks), then
applies (setq + vault sweep) and inserts stubs for the new keys
through the existing apply path. No-op when nothing is missing,
specs are unavailable, or the order file is absent. Returns list
appended."
  (let ((known nil)
        (by-key nil))
      ...
      (maphash (lambda (_k spec)
                 (let ((key (plist-get spec :key)))
                   (when (and (stringp key) (not (string-empty-p key)))
                     (push key known)
                     (push (cons key spec) by-key))))
               my/manifolding-atlas-org-prompt--specs))
    (if (null known)
        (when (file-exists-p (my/manifolding-atlas-drawer-order-file))
          (message "drawer-order: specs not yet loaded, skipping sync (will retry after register-all)"))
      (let* ((order (or (my/manifolding-atlas-drawer-order-from-file) nil))
             (missing (sort (cl-delete-duplicates
                             (seq-remove (lambda (k) (member k order))
                                         known)
                             :test #'string=)
                            #'string<)))
        (when missing
          (let ((f (my/manifolding-atlas-drawer-order-file)))
            (when (file-exists-p f)
              ...
              (my/manifolding-atlas--sweep-sync-drawer-keys
               (delq nil
                     (mapcar (lambda (k)
                               (let ((spec (cdr (assoc k by-key))))
                                 (when spec
                                   (cons k (my/manifolding-atlas-org-prompt--default-for-spec
                                            spec)))))
                             missing)))
              missing)))))))
#+end_src
       ...
       (lambda ()
         (condition-case err
             (let ((n (my/manifolding-atlas-org-prompt--register-all)))
               ...
               (let ((added (my/manifolding-atlas--sync-drawer-order-new-keys)))
                 (when added
                   (message "drawer-order: appended %d missing keys (%s)"
                            (length added) (string-join added ", "))))
               (when (= n 0)
