...
(defun my/manifolding-atlas--all-warning-defaults (context)
  "Return a :unfoldings alist WARNING-defaulting every registered
prompt whose :contexts include CONTEXT (a symbol like 'file).
Reuses the existing per-kind fragment logic so each unfolding is
...
This is the default value of `manifolding-atlas-find-default-create-fn'."
  (let* ((fname (my/manifolding-atlas-prompt-file-name title nil))
         ...
         (default-unfoldings (my/manifolding-atlas--all-warning-defaults 'file))
    ...
    ;; 3. org-capture opens THE CAPTURE BUFFER — visible, title + WARNING already in it
    ...
    (let ((org-capture-templates
      ...
      (my/manifolding-atlas-org-prompt--write-at-heading
       (point-min) 0 nil '("WARNING") "WARNING" nil)
...
After C-c C-c finalizes a new note ("n" template), offer to visit the
file so it doesn't have to be searched for: RET opens it, SPC skips.
One-shot per creation; aborted captures clear the pending path.
#+begin_src emacs-lisp
...
;; Aborted captures never finalize — clear a stale pending path.
...
:KIND: inline|live|todo|multi|tags|template   (default unfolding)
:UNFOLDING:     <family>              (for KIND live — unfolding-note linking family)
...
Inserts it right after the level-1 key heading (past its unfolding
