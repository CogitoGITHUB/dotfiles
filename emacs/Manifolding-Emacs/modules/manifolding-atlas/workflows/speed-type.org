NOTE: this vendored copy may need new features borrowed from the sibling typing packages before Atlas integration — monkeytype's mistyped-words + hard-transitions drills, typit-style timed benchmarks. Design pending.
* Speed-Type — vendored typing trainer
Dakra speed-type v1.7, vendored September 2026 under GPL-3.0-or-later (SPDX kept for license compliance; original authors Gunther Hagleitner, Daniel Kraus). Creator boilerplate (title block, license text, commentary, section banners) deleted by owner decision; functionality untouched. One fenced block per top-level form, grouped below by what it does. Lexical binding preserved via the header cookie.

** 1. Header
Load dependencies and the pre-29.1 compat shim. Nothing else runs at load.

*** Requires and compat

#+begin_src emacs-lisp
;;; speed-type --- vendored typing trainer -*- lexical-binding: t -*-
...
** 2. Customization
Every knob speed-type offers, grouped by concern. Buffer names and drill size first, material sourcing next, behavior and scoring last.
...
** 3. Faces
Correct green, error red, consecutive-error underlined. Overlay faces only.
...
** 4. State
Keymaps and the major mode, a handful of globals, then the buffer-local session variables that carry one drill run.
...
** 5. Statistics engine
WPM/CPM/accuracy/skill math (pure — test these headless), the versioned stats-file format with its upgrade path, and the reporting that reads it back.
...
** 6. Material fetching
Gutenberg, wordlists, pandoc URLs, quotes: download once, cache under speed-type-directory, count frequencies for top-x drills.
...
** 7. Session core
The drill loop itself: setup blueprint, per-keystroke change hooks, pause machinery, preview tape, completion, teardown.
...
** 8. Text preparation
Turn raw material into drill text: transform hooks, preparation, inserters, bounds picking, next-word getters.
...
** 9. Extra-words engine
The trainer whip: errors append words, animated in on timers.
...
** 10. Entry commands
Everything autoloaded. Word drills, text sessions on any material, session actions.
...
** 11. Footer
Feature provision. End of vendored code.
