...
Singleton {
    ...
    Component.onCompleted: {
        if (Settings.isTest) {
            ...
            TerminalManager.displayMessage(`◈ ${authManager._blumePrefix} Protocol::LOCK-SCREEN_GREETD`);
        } else if (Settings.isLockd) {
            _handler = LockdHandler;
            TerminalManager.displayMessage(`◈ ${authManager._blumePrefix} Protocol::LOCK-SCREEN_LOCKD`);
        } else {
            throw new Error("No Auth Manager provided: set LOCK-SCREEN_MODE to 'greetd' or 'lockd'");
        }
