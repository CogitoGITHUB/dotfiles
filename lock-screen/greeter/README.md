...
# hyprland
command = "env HYPRLAND_CONFIG=/etc/lock-screen/greeter.hyprland.conf uwsm start hyprland.desktop"

# niri
command = "env NIRI_CONFIG=/etc/lock-screen/greeter.niri.kdl uwsm start niri.desktop"

# others and desktop environment users
command = "env LOCK-SCREEN_MODE=kiosk cage -ds -m last -- quickshell --path /opt/lock-screen/greeter.qml"
...
command = "cage -ds -- sh -c 'wlr-randr --output HDMI-A-1 --off && env LOCK-SCREEN_MODE=kiosk quickshell --path /opt/lock-screen/greeter.qml'"
...
```bash
LOCK-SCREEN_DEBUG=1 LOCK-SCREEN_MODE=test quickshell --path /opt/lock-screen/greeter.qml
```

### 4. Session Lock (Lockd)

All that's required for the session locker is that you run this command, the `LOCK-SCREEN_MODE=lockd` is what tells the shell to run as a session locker.

```bash
LOCK-SCREEN_MODE=lockd quickshell --path /opt/lock-screen/greeter.qml
```
...
Configuration will be installed in `/etc/lock-screen`. The example file below will be installed by the script and should suit most use cases. Please see the file comments for descriptions.

> _Other Compositors:_ See the options `modes.greetd.launch` and `modes.greetd.exit`

`/etc/lock-screen/greeter.config.json`:

```jsonc
{
  /* Link to the JSON Schema for IDE autocompletion and validation */
  "$schema": "https://raw.githubusercontent.com/TSM-061/lock-screen/main/schema/greeter.schema.json",
...
The lock-screen shell is only a frontend. It does not handle password hashing or sensitive authentication logic itself. It essentially passes your inputs to Greetd or Wayland Session Lock.
...
- **Privilege Level:** The only privileged (root) action required is moving assets to `/opt/lock-screen` and `/etc/lock-screen`.
