...
detect_compositor() {
  ...
  if [[ "$XDG_SESSION_TYPE" != "wayland" ]]; then
    echo
    echo "[ERROR] LOCK-SCREEN only supports Wayland sessions."
    exit 1
  fi
}
...
function install_greeter_compositor_config() {
  ...
  if [[ -z "$config_src" ]]; then
    ...
    echo "      https://github.com/TSM-061/lock-screen/tree/main/greeter#other-environments"
    echo
  fi
